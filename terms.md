---
title: "📜 Terms of Service"
description: "The terms under which The Fairhaven Field Guide is offered — including what's ours, what's Studio Drydock's, and what's simply not our fault."
---
# 📜 Terms of Service

*Effective date: September 14, 2026*

Welcome to The Fairhaven Field Guide ("the Guide", "we"). By reading the Guide,
you agree to these terms. They're short, and most of them boil down to common
sense: this is a free fan-made reference site, offered as-is, with love.

## 1. About the Guide

The Guide is an **unofficial, fan-made** wiki and walkthrough for the video game
*Wylde Flowers*. It is **not affiliated with, endorsed by, or sponsored by
Studio Drydock, Nintendo, Apple, or any other company**. We are fans with a
fondness for pressed flowers and spreadsheet-grade farming data, nothing more.

## 2. Intellectual property

- ***Wylde Flowers***, its characters, story, world, and associated names and
  marks are the property of **Studio Drydock Pty Ltd**. They are referenced here
  solely to identify and describe the game.
- The **original content of the Guide** — its prose, page design, and hand-drawn
  botanical illustrations — is © the Guide's author. You're welcome to share and
  quote it for personal, non-commercial purposes with a link back to the Guide.
  Please don't republish wholesale or use it commercially without permission.
- If you are a rights holder and have a concern about anything on this site,
  contact us at the address below and we will respond promptly.

## 3. No guarantees about game information

The Guide describes a living game. Updates and patches to *Wylde Flowers* may
change mechanics, values, recipes, or story details after a page is written. We
work to keep things accurate, but **all information is provided "as is", without
warranty of any kind**, express or implied — including accuracy, completeness,
or fitness for a particular purpose. If a crop schedule or gift preference here
steers you wrong, we're sorry, but you use the Guide at your own risk.

## 4. Spoilers

Story pages are marked, and the rest of the Guide aims to stay spoiler-light —
but a walkthrough is, by nature, full of things you may not have discovered yet.
Tread carefully; we can't un-spoil the family secret for you.

## 5. Acceptable use

Don't use the Guide, or its hosting infrastructure, to do anything unlawful, to
attempt to disrupt the site, or to scrape content in bulk for republication or
for training commercial products. That's it.

## 6. Third-party links and advertising

The Guide links to external websites and displays third-party advertising (see
our [Privacy Policy]({{ '/privacy.html' | relative_url }})). We don't control
external sites or the content of ads, and linking to something is not an
endorsement of it.

## 7. Limitation of liability

To the fullest extent permitted by law, the Guide and its author will not be
liable for any damages arising out of your use of (or inability to use) the
site. Given that the site is free and consists of farming tips and witchcraft
lore, we trust this will not come up.

## 8. Changes to these terms

We may update these terms from time to time; the effective date above will
change when we do. The site's source lives in a public GitHub repository, so
every revision is publicly visible in the commit history. Continuing to use the
Guide after a change means you accept the updated terms.

## 9. Contact

Questions, corrections, or takedown requests:
**[support@fairhavenfieldguide.com](mailto:support@fairhavenfieldguide.com)**.

<p class="handwritten">signed in pressed ink, somewhere on Fairhaven Island 🌙</p>
